...
- Weather is **not** manually selected — outside temperature and humidity are fetched live from weatherapi.com for `room_config.location` (see `supabase/functions/weather`), not a hot/warm/cool dropdown. The Information page displays the current fetched weather (temp, humidity, condition) for visual confirmation.
- AC unit efficiency (SEER) — defaults to 15 (STANDARD_SEER_V3, a realistic mid-range reference unit) or a custom SEER value in [13.0, 25.0]; global per room, always shown. Frontend inputs on both Information and Simulation pages have the same valid range.
...
- Comfort preference (cold/neutral/warm) — shifts the setpoint ±2°C; see below
...
**Room size → representative m²** (midpoint of each range; used only for the density calculation, not stored/enforced in the schema). Lowered 2026-08-13 from the original 50-150/150-400/400+ "public-space" scale — see the **CoolSense V3** section's note on why:
- Small: 20-40 m² → 30 m²
- Medium: 40-80 m² → 60 m²
- Large: 80+ m² → 120 m²
...
> At the current (post-2026-08-13) m² scale, full mode starts at 5/9/18 people for small/medium/large — a 0-30 people-count range comfortably demos every mode for every room size. (At the original larger scale, this note said a 0-10 slider was too narrow and recommended 0-100; that's no longer necessary but 0-100 still works fine if already wired up.)

**Cooling capacity (BTU/hr) and AC unit efficiency (SEER):** power is derived from physics, not an arbitrary per-mode kW table: `power_kw = required_btu_per_hr ÷ (selected_seer × 1000)` — same formula shape in both V1/V2 and V3, only how `required_btu_per_hr` and the reference SEER are derived differs (V1/V2: per-mode BTU bucket × room-size multiplier, reference SEER 4.5, described below for history; **V3, live**: area+occupancy load, reference SEER 15 — see the **CoolSense V3** section). `required_btu_per_hr` does **not** depend on SEER in either model, matching how a real AC unit's rated BTU/hr capacity is fixed regardless of its efficiency. V1/V2's BTU/hr numbers were derived from the original per-mode kW table at the standard reference SEER (4.5), so "Auto" (SEER 4.5) reproduced the original power numbers exactly — this SEER value never represented a real unit, see V3's section for why it was replaced. A higher SEER (more efficient unit) draws less power for the same BTU/hr; a lower SEER draws more. `calculateAcSettings` returns both `power_kw` and `btu_per_hr` (the latter useful for sizing/selecting a real unit). Stored per room on `room_config.ac_seer` (live default 15, `STANDARD_SEER_V3`; valid range [13.0, 25.0] since 2026-08-13, matching real-world manufacturer SEER ranges) and returned on `ac_calculations.btu_per_hr`.
...
- `POST /simulation/run` (`{ duration_hours?, room_size?, ac_seer?, weather_condition?, static_temp_c?, comfort_preference?, schedule_start_hour?, schedule_end_hour? }`, defaults 168/medium/4.5/warm/25/neutral/unset/unset) reads the most recent `duration_hours` mock readings (400s if not enough exist yet — generate mock data first) and compares:
...
- **Optional operating-hours schedule** (`schedule_start_hour`/`schedule_end_hour`, added 2026-08-14, **simulation-only** — deliberately not wired into the live `/calculation` endpoint, since a schedule can't be meaningfully demoed against a single live reading the way the 168h comparison can): when both are set (0-23, either can be given without the other rejected as a 400), every one of the four series (static, CoolSense V2, static-v3, CoolSense V3) draws zero power outside `[schedule_start_hour, schedule_end_hour)` — an overnight-wrapping window (e.g. 22→6) is supported. Applied uniformly to all models so the comparison stays apples-to-apples: it's modeling a physical on/off timer on the unit, not a control-strategy trick. `runSimulation`'s pure-function signature takes an optional `{ startHour, endHour }` 8th argument (`OperatingHoursSchedule`) and requires `capturedAt` timestamps when set (same requirement as `diurnal` weather, for the same reason — needs each hour's actual clock time). Stored on `simulation_runs.schedule_start_hour`/`schedule_end_hour` (nullable) for traceability, same pattern as `static_temp_c`/`comfort_preference`. Tested with realistic 168h data: a schedule beats CoolSense V3 running unscheduled 24/7 by a wide margin (CoolSense's envelope-load floor never hits true zero), but once both are put on the *same* schedule, CoolSense V3 is still ~12% more efficient on top of it — see `PITCH_PREP.md`'s Q&A for the exact framing to use with judges.
- **Per-hour setpoint tracking** (`static_v3_temperature_c`/`coolsense_v3_temperature_c` on `SimulationHourResult`/`simulation_hourly_data`, added 2026-08-14, for the Simulation page's temperature-over-time graph): `coolsense_v3_temperature_c` is `calculateCoolSenseV3Settings`'s `adjusted_temp_c` for that hour; `static_v3_temperature_c` is the static-v3 baseline's effective (weather/setpoint-clamped) temperature, computed once and constant across the whole run — a naive system doesn't adapt. Both stay populated (not zeroed) during scheduled-off hours, since they represent "what the thermostat is set to," not "what's currently drawing power." `LineAreaChart.vue` gained an optional `yMin` prop (default 0) so this chart can float its axis near the realistic 18-26°C setpoint range instead of being forced to start at 0, which would compress the whole visible swing into a sliver at the top — existing power/energy chart callers are unaffected (`yMin` defaults to the prior hardcoded-0 behavior).
...
**CoolSense V2** (`supabase/functions/_shared/coolSenseV2Calculation.ts` — used by `/simulation/run`'s "CoolSense V2" comparison series; no longer the live model, see **CoolSense V3** below for what `/calculation` actually runs today):
...
**CoolSense V3** (`supabase/functions/_shared/acCalculationV3.ts` + `coolSenseV3Calculation.ts`, **live** — this is what `/calculation` and the camera-triggered auto-recalculation (`_shared/runCalculation.ts`) actually run, as of 2026-08-13; also still used in `/simulation/run`'s comparison):
...
- **Room-size rescale (2026-08-13):** at the original 50-150/150-400/400+ m² "public-space" scale, V3's realistic BTU/hr sizing meant moderate/full occupancy in a medium/large room required 50,000-100,000+ BTU/hr — far beyond a single real AC unit's typical range (residential/light-commercial splits top out around 24,000-60,000 BTU/hr), which read as unrealistic in a pitch ("a 12,000 BTU unit can't handle 40 people"). `ROOM_SIZE_SQM`/`ROOM_SIZE_SQM_RANGES` in `acCalculation.ts` were lowered (see "Room size → representative m²" above) so each room size's full-mode capacity requirement (small ~6,500-15k, medium ~12.6k-25k+, large ~25k-35k+ BTU/hr) stays servable by one plausible real unit. Multi-AC-unit support (making `room_config` a real multi-row table) was considered as the alternative fix but is a much bigger rework (schema + every function assuming the `id=1` singleton) than rescaling these constants — deferred as future work, not attempted.
...
- `/simulation/run` computes CoolSense V3 alongside a size-aware **static-v3** baseline (not the flat `CURRENT_SYSTEM_POWER_KW` V2 compares against): static-v3 is sized for the room's full-mode occupancy *and* the run's worst-case weather load for the given `weather_condition` (the flat preset's own temp/humidity, or diurnal's 3pm peak of 36°C/80%RH), and its effective setpoint can't be milder than full mode's own base temp (21°C) — `static_temp_c` can still push it colder, raising power further, but can't make it complacently warmer than what real peak occupancy demands. This keeps the comparison physically honest once capacity scales with room size (a flat baseline would otherwise make "smart" look worse than "dumb" during peak hours in large rooms). Sizing static-v3 only against the *baseline* weather (33°C/60%RH, multiplier 1) regardless of the run's actual `weather_condition` was a bug (fixed 2026-08-13): CoolSense V3's real per-hour weather multiplier could then exceed this frozen capacity under `hot`/`diurnal`-peak conditions at/near full occupancy, showing the "smart" system using *more* power than the baseline — see `simulation.test.ts`'s "never exceeds the static-v3 baseline ... under hot/diurnal peak weather" tests. The sizing occupancy also carries a **1.2x headroom margin** (`STATIC_V3_SIZING_HEADROOM` in `simulation.ts`, added 2026-08-14) above the exact full-mode threshold: `generateMockOccupancy`'s own ±15% noise can realistically push a peak reading above that exact boundary (e.g. medium room: threshold 9 people, noise ceiling ceil(9×1.15)=11), and without headroom that was enough to make CoolSense exceed an unmargined baseline — stress-tested across 90 simulated weeks with the app's real noise distribution, ~67% had at least one such hour before the fix, 0% after. This also *raised* the reported savings number, since static-v3 is a flat per-run power draw applied to every hour, not just crowded ones — undersizing it understated the baseline's draw across the whole simulation, not just during the violating hours; overall reduction moved from ~25.5% to ~30.0% on the same 90-week check purely from correcting this. 1.2x covers the realistic noise band with buffer; it does not (and no fixed multiplier could) cover arbitrary overcrowding far beyond "full" — a room genuinely packed 50%+ over capacity is a real capacity problem no fixed AC sizing solves either. `simulation_runs`/`simulation_hourly_data` store the V3 fields alongside the existing static/V2 ones (`static_v3_energy_kwh`, `coolsense_v3_energy_kwh`, `v3_pct_reduction`, etc.) — additive columns, nothing existing changed.
- `tools/calculation-tester.html`'s simulation charts show CoolSense V3 as a third line/area alongside static and V2.
- **AC unit capacity ceiling** (`_shared/runCalculation.ts`'s `applyCapacityCeiling`, live only — not part of `/simulation/run`): `room_config.rated_capacity_btu_per_hr` (nullable; the Information page's "Cooling Capacity" field) is the *actual* rated capacity of the installed unit, distinct from V3's *computed* `required_btu_per_hr`. If set and lower than the computed requirement, the unit can't keep up — `power_kw` is capped at what that unit can actually draw (`rated_capacity_btu_per_hr ÷ (seer × 1000)`) rather than reporting a draw the unit couldn't produce, and the row is flagged `ac_calculations.capacity_constrained = true` (surfaced in the People page's AC Setting card as an "undersized" warning). Unset (the common case) is a no-op — behavior is identical to not having this field at all.

**`room-config` GET/PUT** (`supabase/functions/room-config`): `GET` returns the singleton row as-is (404 only if the seed row is somehow missing). `PUT` (POST also still accepted, for backward compatibility) validates and partially updates: `room_size` ∈ {small,medium,large}; `ac_seer` ∈ [13.0, 25.0] (real-world manufacturer SEER range, matching CoolSense V3's `STANDARD_SEER_V3 = 15` — this range changed from the old V1/V2-era [2.0, 6.0] on 2026-08-13 when the live calculation switched to V3, see below); `rated_capacity_btu_per_hr` — positive number or null, the unit's actual rated capacity (see CoolSense V3's capacity ceiling below); `egat_label` ∈ {'1'-'5','premium', null}, only settable when the resulting `location` is Thailand (rejects otherwise, and auto-clears a stored label if `location` moves away from Thailand in a request that doesn't touch the label); `comfort_preference` ∈ {cold,neutral,warm}.
