...
**Cooling capacity (BTU/hr) and AC unit efficiency (SEER):** power is derived from physics, not an arbitrary per-mode kW table: `power_kw = required_btu_per_hr ÷ (selected_seer × 1000)` — same formula shape in both V1/V2 and V3, only how `required_btu_per_hr` and the reference SEER are derived differs (V1/V2: per-mode BTU bucket × room-size multiplier, reference SEER 4.5, described below for history; **V3, live**: area+occupancy load, reference SEER 15 — see the **CoolSense V3** section). `required_btu_per_hr` does **not** depend on SEER in either model, matching how a real AC unit's rated BTU/hr capacity is fixed regardless of its efficiency. V1/V2's BTU/hr numbers were derived from the original per-mode kW table at the standard reference SEER (4.5), so "Auto" (SEER 4.5) reproduced the original power numbers exactly — this SEER value never represented a real unit, see V3's section for why it was replaced. A higher SEER (more efficient unit) draws less power for the same BTU/hr; a lower SEER draws more. `calculateAcSettings` returns both `power_kw` and `btu_per_hr` (the latter useful for sizing/selecting a real unit). Stored per room on `room_config.ac_seer` (live default 15, `STANDARD_SEER_V3`; valid range [13.0, 25.0] since 2026-08-13) and returned on `ac_calculations.btu_per_hr`.
